...
class CPDatasetTest(data.Dataset):
    ...
    """
    def __init__(self, opt, person_img_name, cloth_img_name):
        ...
        self.im_names = [person_img_name]
        self.c_names = dict()
        self.c_names['paired'] = [cloth_img_name]
        self.c_names['unpaired'] = [cloth_img_name]
    ...
    def get_im_agnostic(self, im, im_parse, pose_data, w=768, h=1024):
    ...
    def __getitem__(self, index):
        ...
        for key in self.c_names:
            ...
            # 원본 상품 이미지
            c[key] = Image.open(osp.join(self.data_path, 'raw_data/cloth', c_name[key])).convert('RGB')
            c[key] = transforms.Resize((self.fine_height, self.fine_width), interpolation=2)(c[key])
            c[key] = self.transform(c[key])  # [-1,1]

            # 원본 상품 마스크 이미지
            cm[key] = Image.open(osp.join(self.data_path, 'preprocess/mask/cloth', c_name[key]))
        ...
        # 사람 이미지
        im_pil_big = Image.open(osp.join(self.data_path, 'raw_data/person', im_name))
        ...
        parse_label = ["Background", "Hat", "Hair", "Glove", "Sunglasses",
                       "Upper-clothes", "Dress", "Coat", "Socks", "Pants",
                       "tosor-skin", "Scarf", "Skirt", "Face", "Left-arm",
                       "Right-arm", "Left-leg", "Right-leg", "Left-shoe", "Right-shoe"]

        # parse label을 재정의하기 위해 사용
        labels = {
            0:  ['background',  [0, 10]],
            1:  ['hair',        [1, 2]],
            2:  ['face',        [4, 13]],
        ...
        # new_parse_map은 하나의 채널에 있던 parse label들을
        # semantic_nc개의 채널로 각각 분리해둔 map이며
        # object가 있는 경우 1.0이 입력됨.
        for i in range(len(labels)):
            for label in labels[i][1]:
                new_parse_map[i] += parse_map[label]
        
        # metric 계산을 위해 추출하는 것으로 보임
        ...
        # 사람 이미지에서 옷 부분을 추출하는 코드
        # pcm: 상품의 parse mask, im_c: pcm을 이용해서 검정 배경에 상품 부분만 남겨둠.
        pcm = new_parse_map[3]
        im_c = im * pcm + (1 - pcm)
        ...
        # 여기가 문제 -> (768, 1024) 이미지로 만들려면 keypoint도 그만큼 곱해줘야 하는데 오차가 발생할 것 같음.
        ...
        with open(osp.join(self.data_path, 'preprocess/pose/keypoints', pose_name), 'r') as f:
            ...
            pose_data = pose_data.reshape((-1, 3))[:, :2] # (25, 2): (키포인트 개수, 좌표)
        ...
        # parse agnostic 얻기
        image_parse_agnostic = get_im_parse_agnostic(im_parse_pil, pose_data)
        ...
        # person agnostic 얻기
        agnostic = self.get_im_agnostic(im_pil, im_parse_pil, pose_data)
